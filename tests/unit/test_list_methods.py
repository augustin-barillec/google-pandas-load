...
class ListMethodsTest(BaseClassTest):
    ...
    def test_list_blob_uris(self):
        ...
        self.assertEqual(
            ...
            sorted(['gs://{}'.format(bucket_name) + '/a{}_gs'.format(i) for i in range(9, 12)]))

        self.assertEqual(
            sorted(gpl1.list_blob_uris(data_name='a1')),
            sorted(['gs://{}'.format(bucket_name) + '/a{}_gs'.format(i) for i in range(10, 12)]))

        self.assertEqual(
            sorted(gpl2.list_blob_uris(data_name='a')),
            sorted(['gs://{}'.format(bucket_name) + '/dir/subdir/a{}_gs'.format(i) for i in range(7, 11)]))

        self.assertEqual(
            sorted(gpl2.list_blob_uris(data_name='a1')),
            sorted(['gs://{}'.format(bucket_name) + '/dir/subdir/a{}_gs'.format(i) for i in range(10, 11)]))
