...
class ListMethodsTest(BaseClassTest):
    ...
    def test_list_blob_uris(self):
        blobs = [storage.Blob(name='a{}'.format(i), bucket=bucket) for i in range(9, 12)]
        for b in blobs:
    ...
    def test_list_local_file_paths_when_no_data_named_data(self):
        with open(os.path.join(local_dir_path, 'a0'), 'w') as outfile:
            json.dump('data', outfile)

        expected_res = []

        res = gpl3.list_local_file_paths(data_name='b')
